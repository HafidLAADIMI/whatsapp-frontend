...
const Contact = () => {
  ...
  const deduplicateMessages = (existingMessages, newMessages) => {
   ...
   const getBackupMessages = async (senderId:string,receiverId: string) => {
    try {
      const response = await axios.get(
        `http://172.16.228.52:8080/message/getChatMessages/${senderId}/${receiverId}`
      );
  ...
  useEffect(() => {
    ...
    const client = new Client({
      brokerURL: "ws://172.16.228.52:8080/ws",
        ...
        client.subscribe(`/user/${userId}/private`, (message) => {
          ...
          try {
            ...
            console.log(
              ...
              Object.keys(receivedMessage)
            );
            console.log("Received message:", receivedMessage);
            setMessages((prev) => deduplicateMessages(prev, [receivedMessage]) as never);
            console.log("messages after receiving message : " + messages);
          } catch (error) {
            console.error("Error parsing message:", error);
          }
        });
        getBackupMessages(userId, receiverId);
      },
      onDisconnect: () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      await axios.post("http://172.16.228.52:8080/message/send", newMessage, {
        headers: {
  ...
  const getByReceiverId=async(receiverId:string)=>{
    try{
       const response=await axios.get(`http://172.16.228.52:8080/user/getById/${receiverId}`);
      ...
      <View className="h-full w-full flex flex-col">
        <View className="flex flex-row items-center justify-between w-full py-3 bg-[#131814] px-2 border-b-[1px] border-[#181d23]">
          ...
          <View className="flex flex-row gap-3 items-center">
            ...
            />
            <Image
              source={{ uri: receiver?.profile }}
              className="w-10 h-10 rounded-full"
            />
            <Text className="text-[17px] text-white font-bold">{receiver?.username}</Text>
          </View>
          {/* Action Buttons */}
